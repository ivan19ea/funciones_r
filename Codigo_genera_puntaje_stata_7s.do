...
syntax, tol(numlist) priorp(numlist) save(string) score(string)
	...
	* Iterations
	local h = 0
	while `tolm1'==0 | `tolm2'==0 | `tolm3'==0 | `tolm4'==0 | `tolm5'==0 | `tolm6'==0 | `tolm7'==0  | `tolu1'==0 | `tolu2'==0 | `tolu3'==0 | `tolu4'==0 | `tolu5'==0 | `tolu6'==0 | `tolu7'==0 {
		local h = `h'+1
		local r = `h'-1
		di "Iteration `h':"
		* E Step
		replace `g' = `p'*((`m1'^s1)*(1-`m1')^(1-s1))*((`m2'^s2)*(1-`m2')^(1-s2))*((`m3'^s3)*(1-`m3')^(1-s3))*((`m4'^s4)*(1-`m4')^(1-s4))*((`m5'^s5)*(1-`m5')^(1-s5))*((`m6'^s6)*(1-`m6')^(1-s6))*((`m7'^s7)*(1-`m7')^(1-s7)) 
		replace `g' = `g'/(`g'+(1-`p')*((`u1'^s1)*(1-`u1')^(1-s1))*((`u2'^s2)*(1-`u2')^(1-s2))*((`u3'^s3)*(1-`u3')^(1-s3))*((`u4'^s4)*(1-`u4')^(1-s4))*((`u5'^s5)*(1-`u5')^(1-s5))*((`u6'^s6)*(1-`u6')^(1-s6))*((`u7'^s7)*(1-`u7')^(1-s7))   )
		* M Step
		* p parameter
			sum `g'	
			local p = r(mean)
	...
	* Calculate score 
	gen double `score' = s1*`w1m'+(1-s1)*`w1u' + s2*`w2m'+(1-s2)*`w2u' + s3*`w3m'+(1-s3)*`w3u' + s4*`w4m'+(1-s4)*`w4u' + s5*`w5m'+(1-s5)*`w5u' + s6*`w6m'+(1-s6)*`w6u' + s7*`w7m'+(1-s7)*`w7u'  
end
