...
ENSANUT <- sqlQuery(con, "SELECT top 8000 id, primer_NOMBRE, SEGUNDO_NOMBRE, PRIMER_APELLIDO, SEGUNDO_APELLIDO FROM  [vincula_yelda_gdl_paz].[dbo].[gdl]")
...
INEGI <- sqlQuery(con, "select top  250000 id, nombre_mod, paterno_mod, materno_mod  from [vincula_imss_yelda].[dbo].[seed_2017_2022]")
...
#Respaldar block2 antes de modificarlos
recupurar <- block2

#Respaldar l que se ha generado de bloqueo
bloque_p1_2018 <-  bloqueo_fin




#Modificar bloque 2, solo quedarnos con los blocks que aun no han pasado
remplazar <- block2[ which(block2$id>23) , ] 

#agregar id consecutivo
remplazar$id <- 1:5

#Asignar el resultado a block2 y comenzar a vincular
block2 <-  remplazar
